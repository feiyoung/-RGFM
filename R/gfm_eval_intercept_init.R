...
paraglmfit <- function(j, Xx, g1, XX, fun1){
  glm.fit(x=Xx, y=XX[, g1[j]], family = fun1, intercept = F)$coefficients
}
localupdateB2 <- function(X, g1, hH, type1, parallel=F){
  ...
  if(parallel){
    ...
    for(j in jg){
      ...
      B1[,j] <- glm.fit(x=cbind(1,hH), y=X[, g1[j]], family = fun1, intercept = F)$coefficients
    }
  }
...
paraglmj1fit <- function(i,j, hBm, gcell, w, XX, funj){
  glm.fit(hBm[gcell[[j]], -1], XX[i, gcell[[j]]], weights = w,intercept = F,
          family=funj,offset = hBm[gcell[[j]], 1])$coefficients

}
paraglmj2fit <- function(i,j, hBm, gcell, XX, funj){
  glm.fit(hBm[gcell[[j]], -1], XX[i, gcell[[j]]], intercept = F,
          family=funj,offset = hBm[gcell[[j]], 1])$coefficients

}
localupdateH2 <- function(X, gcell, hBm, type, dropout, parallel=F){
  ...
  for(jj in 1: length(idres)){
    ...
    if(type[j] == 'gaussian'){
      ...
      if(parallel){
        ...
        for(i in 1:n){ #i <- 1
          H2[,i] <- glm.fit(hBm[gcell[[j]], -1], X[i, gcell[[j]]], weights = w,intercept = F,
                            family=funj,offset = hBm[gcell[[j]], 1])$coefficients
        }
      }
      ...
      if(parallel){
        ...
        for(i in 1:n){
          try(
            H2[,i] <- glm.fit(hBm[gcell[[j]], -1], X[i, gcell[[j]]],intercept = F,
                              family=funj,offset = hBm[gcell[[j]], 1])$coefficients
            , silent=T
          )
        }
...
gfm_eval_intercept_init <- function(X, group, type, q,
  ...
  omega <- 1/p
  #initialize
  hH <- Factorm(scale(X,scale=F), q)$hH;hB <- 0
  eps1 <- 1e-4
