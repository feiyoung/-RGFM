gfm <- function(X, group, type, q=NULL, parallel=T,para.type='doSNOW', ncores = 10,
                dropout=0, dc_eps=1e-4, maxIter=50,
                q_set=1:10, output=T, fast_version=F){
      ...
      resq <- foreach(r = 1:nq,.packages="GFM" ,.options.snow=opts,
                        .combine='rbind') %dopar% {
                          res <- paraIC(r, X, group, type,
                                                        dropout=dropout, eps2=1e-4, maxIter=10, output=output, fast_version=T)
                          res
                        }
      close(pb)
      stopCluster(cl)
